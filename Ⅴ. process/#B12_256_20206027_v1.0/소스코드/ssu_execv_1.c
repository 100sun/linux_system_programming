...
int main(void)
{
    ...
    if ((pid = fork()) == 0) // if it is a child process
    {
        char *args[] = {"find", "/", "-maxdepth", "4", "-name", "stdio.h", NULL};
        if (execv("/usr/bin/find", args) < 0) // restart a new program -> to main()
        { // error
    ...
    if (wait3(&status, 0, &rusage) == pid) // wait of any child
